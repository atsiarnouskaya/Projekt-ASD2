    ...
    public static void generateSVGfile(ArrayList<Road> roads, ArrayList<Farmland> farmlands, ArrayList<Brewery> breweries, ArrayList<Tavern> taverns) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter("map.svg"))) {
            ...
            for (Road road : roads) {
                ...
                writer.println("<text x=\"" + String.format("%.0f", (road.x1 + road.x2) / 2) + "\" y=\"" + String.format("%.0f", (road.y1 + road.y2) / 2) + "\" fill=\"white\" font-family=\"Arial, sans-serif\" font-weight=\"600\" font-size=\"3.5\" text-anchor=\"middle\" dominant-baseline=\"middle\">" + road.getMaxBeerFlow() + "/" + road.getMaxBarleyFlow() + "</text>");
            }
